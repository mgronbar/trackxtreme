...
public class LibraryListActivity extends ListActivity implements OnSharedPreferenceChangeListener {
	...
	private void fillData() {
		...
		for (Track book : getLibrary().getTracks()) {
			// Stringtle = book.getTitle();
			// bookNam ties.add(title);
		}
		String[] items = new String[bookNames.size()];
		bookNames.toArray(items);
		// ArrayAdapter<String> books = new ArrayAdapter<String>(this,
		// R.layout.book_row, R.id.text1, items);
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		// startActivity(iSemenu_insertttings);
		// return true;
		}
